...
|------|--------|--------|------------|------|-----------|-------------------|-------|---------------|-----------|------------|----------|
| 1 | `buffer` | `buffer.BufferModule [ZERO] [PROVENANCE-FALLBACK]` | 0.00 | 12 | 0/0 matched | _none_ | 0/0 matched | _none_ | 0 | 0 | 12000010.0 |
...
| 5 | `rect.iter` | `rect.Iter` | 0.81 | 5 | 16/16 matched (target 25) | _none_ | 3/4 matched | `Item` | 1 | 20 | 5012002.0 |
...
| 15 | `buffer.cell_width` | `buffer.CellWidth [ZERO] [PROVENANCE-FALLBACK]` | 0.00 | 2 | 4/4 matched | _none_ | 1/1 matched (target 2) | _none_ | 0 | 5 | 2000510.0 |
...
| 27 | `backend.test` | `backend.TestBackend [ZERO] [PROVENANCE-FALLBACK]` | 0.00 | 0 | 29/47 matched (target 73) | `fmt`, `buffer_view_with_overwrites`, `assert_buffer_panics`, `assert_scrollback_panics`, `display`, `clear_region_all`, `clear_region_after_cursor`, `clear_region_before_cursor`, `clear_region_current_line`, `clear_region_until_new_line`, `append_lines_not_at_last_line`, `append_lines_at_last_line`, `append_multiple_lines_not_at_last_line`, `append_multiple_lines_past_last_line`, `append_multiple_lines_where_cursor_at_end_appends_height_lines`, `append_multiple_lines_where_cursor_appends_height_lines`, `append_multiple_lines_where_cursor_at_end_appends_more_than_height_lines`, `append_lines_truncates_beyond_u16_max` | 1/3 matched (target 2) | `Result`, `Error` | 20 | 50 | 205010.0 |
| 28 | `symbols.border` | `border.Border [PROVENANCE-FALLBACK]` | 0.08 | 0 | 2/22 matched (target 2) | `render`, `border_set_from_line_set`, `plain`, `rounded`, `double`, `thick`, `light_double_dashed`, `heavy_double_dashed`, `light_triple_dashed`, `heavy_triple_dashed`, `light_quadruple_dashed`, `heavy_quadruple_dashed`, `quadrant_outside`, `quadrant_inside`, `one_eighth_wide`, `one_eighth_tall`, `proportional_wide`, `proportional_tall`, `full`, `empty` | 1/1 matched (target 3) | _none_ | 20 | 23 | 202309.2 |
| 29 | `terminal.cursor` | `terminal.CursorTest [PROVENANCE-FALLBACK]` | 0.39 | 0 | 5/11 matched (target 5) | `hide_cursor`, `show_cursor`, `get_cursor`, `set_cursor`, `get_cursor_position`, `set_cursor_position` | 0/0 matched (target 1) | _none_ | 6 | 11 | 61106.1 |
| 30 | `rect.ops` | `rect.Ops [PROVENANCE-FALLBACK]` | 0.24 | 0 | 4/9 matched (target 15) | `neg`, `add`, `sub`, `add_assign`, `sub_assign` | 0/1 matched | `Output` | 6 | 10 | 61007.6 |
| 31 | `terminal.backend` | `backend.ClearTypeTest [PROVENANCE-FALLBACK]` | 0.00 | 0 | 0/6 matched (target 2) | `backend`, `backend_mut`, `size`, `backend_returns_shared_reference`, `backend_mut_allows_mutating_backend_state`, `size_queries_underlying_backend_size` | 0/0 matched (target 1) | _none_ | 6 | 6 | 60610.0 |
| 32 | `symbols.merge` | `merge.Merge [ZERO] [PROVENANCE-FALLBACK]` | 0.00 | 0 | 6/10 matched (target 11) | `new`, `replace_merge_strategy`, `exact_merge_strategy`, `fuzzy_merge_strategy` | 3/4 matched (target 3) | `BorderSymbolError` | 5 | 14 | 51410.0 |
| 33 | `symbols.line` | `line.Line [PROVENANCE-FALLBACK]` | 0.14 | 0 | 1/6 matched (target 3) | `render`, `normal`, `rounded`, `double`, `thick` | 1/1 matched (target 3) | _none_ | 5 | 7 | 50708.6 |
| 34 | `buffer.buffer` | `buffer.Buffer [STUB] [PROVENANCE-FALLBACK]` | 0.00 | 0 | 60/63 matched (target 85) | `fmt`, `small_one_line_buffer`, `diff_empty_filled` | 1/2 matched (target 4) | `Output` | 4 | 65 | 46510.0 |
...
| 42 | `symbols.bar` | `bar.Bar [PROVENANCE-FALLBACK]` | 0.99 | 0 | 1/1 matched (target 16) | _none_ | 1/1 matched (target 2) | _none_ | 0 | 2 | 200.1 |
| 43 | `symbols.block` | `block.Block [PROVENANCE-FALLBACK]` | 0.99 | 0 | 1/1 matched (target 43) | _none_ | 1/1 matched (target 4) | _none_ | 0 | 2 | 200.1 |
...
| 45 | `palette.tailwind` | `tailwind.Tailwind [PROVENANCE-FALLBACK]` | 1.00 | 0 | 0/0 matched | _none_ | 1/1 matched | _none_ | 0 | 1 | 100.0 |
| 46 | `symbols.half_block` | `symbols.HalfBlock [PROVENANCE-FALLBACK]` | 1.00 | 0 | 0/0 matched | _none_ | 0/0 matched (target 1) | _none_ | 0 | 0 | 0.0 |
| 47 | `symbols.braille` | `symbols.Braille [PROVENANCE-FALLBACK]` | 1.00 | 0 | 0/0 matched | _none_ | 0/0 matched (target 1) | _none_ | 0 | 0 | 0.0 |
| 48 | `symbols.shade` | `shade.Shade [PROVENANCE-FALLBACK]` | 1.00 | 0 | 0/0 matched | _none_ | 0/0 matched (target 1) | _none_ | 0 | 0 | 0.0 |
| 49 | `symbols.pixel` | `symbols.Pixel [PROVENANCE-FALLBACK]` | 1.00 | 0 | 0/0 matched | _none_ | 0/0 matched (target 1) | _none_ | 0 | 0 | 0.0 |
...
- **buffer** → `buffer.BufferModule [ZERO] [PROVENANCE-FALLBACK]`
  ...
  - Scoring failure: BufferModule.kt: score-padding suppression annotation `@Suppress` in Kotlin code
  - Lint issues: 1
...
- **layout.rect** → `layout.Rect [ZERO]`
  ...
  - Missing types: _none_
  - Scoring failure: RectUnitTest.kt: snake_case identifier `inside_top_left` in Kotlin comments
...
- **buffer.cell_width** → `buffer.CellWidth [ZERO] [PROVENANCE-FALLBACK]`
  - Function similarity: 0.00
  - Dependencies: 2
  ...
  - Missing types: _none_
  - Scoring failure: CellWidth.kt: snake_case identifier `ratatui_core` in Kotlin comments
  - Lint issues: 2
